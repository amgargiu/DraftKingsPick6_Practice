...
//
//  Created by Antonio Gargiulo on 3/22/26.
//
...
//    }
    ...
    func get(imageName: String, folderName: String) -> Data? {
        // 1. Get the URL path to the file
        guard let path = imagePath(imageName: imageName, folderName: folderName) else { return nil }
        
        // 2. Check if the file actually exists
        guard FileManager.default.fileExists(atPath: path.path) else { return nil }
        
        print("FM: getting raw data from disk")
        
        // 3. RETURN DATA, NOT UIIMAGE
        // This reads the bytes directly without "inflating" the image into RAM
    ...
    func deleteFolder(folderName: String) {
        ...
        do {
            ...
            print("deleted folder succesfully")
        } catch let error {
            print("error deleting file \(error)")
        }
    }
